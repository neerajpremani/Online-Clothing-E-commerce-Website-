...
<?php
		...
		if(!isset($_SESSION['email']) ||(!isset($_SESSION['pwd'])))
			...
			else
			{
				...
				$query1=mysqli_query($con,"select quantity from product where product_code='$code'");
				...
				if($r>0)
				{
					$querycheck=mysqli_query($con,"select quantity,product_code,product_name from shopping_cart where email='".$email."' and product_code='$code'");
					$rt=mysqli_fetch_array($querycheck);
					$rr=$rt['quantity'];
					$abc=$rt['product_code'];
					$def=$rt['product_name'];
					...
					$queryup=mysqli_query($con,"update shopping_cart set quantity='$rr'+'$quantity' where product_code='$abc'");
						$query2=mysqli_query($con,"update product set quantity='$r' where product_code='$code'");
				...
				$rsadd=mysqli_query($con,$queryadd);
				
				$querycopy="insert into history_acc values('','$email','$code','$pro_name','$image','$quantity','$price','')";
				$rscopy=mysqli_query($con,$querycopy);
				
				$querycopy1="insert into cartdetails values('','$email','$code','$pro_name','$image','$quantity','$price','')";
				$rscopy1=mysqli_query($con,$querycopy1);
				if($rsadd)
				 { 
					 header('location:cartdetails.php');
				...
				{
					 echo "<script>alert('Cannot add.Insufficient Storage in the DATABASE');</script>";	
				}
...
<a href="cartdetails.php">
         ...
         <input type="submit" name="submit" value="ADD TO CART" style="border:0px solid #000000; border-radius:10px; height:40px; width:140px; background-color:#7CE1D6; font:bold; ">
          ...
          <tr><td style="background-color:#F6F6F6; width:300px; font-size:15px;">Ratings</td><td><?php echo $row['Ratings'];?></td></tr>
